    ...
    public class State_AutoLoad : IState
    {
        ...
        public void Handle(ClientRequestHandler handler)
        {
            try
            {
                ...
                log.Debug(m => m("[State_AutoLoad] Request(ASCII):{0}", ClientRequestHandler.asciiOctets2String(handler.Request)));
                //1.驗證Mac
        ...
        protected virtual AL2POS_Domain GetResponse(AL2POS_Domain request)
        {
            ...
            try
            {
                ...
                string[] setting = ConfigLoader.GetSettings(ConType.AutoLoad).Split(':');
                string ip = setting[0];
