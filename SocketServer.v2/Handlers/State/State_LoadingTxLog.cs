    ...
    public class State_LoadingTxLog :IState
    {
        ...
        public void Handle(ClientRequestHandler handler)
        {
            try
            {
                ...
                {
                    //1.parse Txlog
                    request = this.ParseRequest(CheckMacContainer.LOLTxLogReqMsgUtility, handler.Request);
                ...
                {
                    log.Error(m => m("[State_LoadingTxLog][Handle] Business Error: {0} \r\n{1}", ex.Message, ex.StackTrace));
                    //格式 error
                    response = new Txlog_Domain()
                    {
                        TXLOG_RC = "770001"
                ...
                //3.開始轉換Response
                responseArr = this.ParseResponse(CheckMacContainer.LOLTxLogRespMsgUtility, response, handler.Request);
            ...
            {
                log.Error(m => m("[State_LoadingTxLog][Handle] Error: {0} \r\n{1}", ex.Message, ex.StackTrace));
            }
            finally
            {
